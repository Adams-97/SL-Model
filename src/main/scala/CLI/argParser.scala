...
import Domain.UnknownArgError.checkUnknownErr

object argParser {

  def parseArgs(arrOfArgs: Array[String]): Either[CLIError,Seq[CLIArg]]= {
    val listArgs = checkForNoArgs(arrOfArgs.toVector)
    val helpcheck = listArgs.flatMap(checkForHelpArg)
    val CLIArgSeq = helpcheck.flatMap(strArgs => getCLIArg(strArgs))
    val mandatoryCheck = CLIArgSeq.flatMap(checkMandatoryArgs)
    mandatoryCheck.flatMap(checkUnknownErr)
  }
  ...
  def checkForHelpArg(args: Seq[String]): Either[HelpArgError,Seq[String]] = {
